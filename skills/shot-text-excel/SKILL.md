...
For short bottom-subtitle product videos around `40-50s`, keep the fast path unless validation finds real alignment errors: model report + `--subtitle-region bottom` + `--targeted-ocr-frame-budget 48` should be enough when every shot column has a plausible subtitle. Do not create extra contact sheets or recalibrate cuts just because adjacent subtitles repeat. Spot-check only obvious risk rows, then clean text in-place.
...
For underwear package/display shots, if OCR joins package or brand print with the real bottom subtitle, keep only the spoken subtitle after the slash/noise prefix. Examples from 6.30 short underwear footage: `LUCKMEEY幸棉/真的就是纯纯的捡大漏` -> `真的就是纯纯的捡大漏`, `rnCKWEE人幸/像内裤这种消耗品` -> `像内裤这种消耗品`, and `其他内裤/而且它整个做的是一个全包裹的臀围` -> `而且它整个做的是一个全包裹的臀围`. Do not rerun OCR for these obvious prefix trims; patch the workbook copy row directly.
