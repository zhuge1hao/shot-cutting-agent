...
VIDEO_PATH = next(
    path
    for path in (ROOT / "videos" / "test").iterdir()
    if path.is_file() and path.name.startswith("5.9+") and "嫦香诗" in path.name
)
...
THUMB_DIR = OUTPUT_EXCEL_DIR / "_thumbs" / "bra_changxiangshi_5_9"
EXCEL_PATH = OUTPUT_EXCEL_DIR / "bra_changxiangshi_5_9_shot_text.xlsx"
...
def classify_plan(index: int, shot: dict, copy: str, repeated_copy: bool) -> dict:
    ...
    elif "香诗" in text or "聚拢内衣" in text:
        visual = "聚拢内衣正式露出"
        framework = "产品给到解决方案"
        viewer = "明确产品是聚拢型文胸，开始判断能否解决胸型松散和下垂问题。"
    ...
    elif any(word in text for word in ["旧内衣", "全换", "换成"]):
        visual = "旧内衣换新痛点"
        framework = "换新理由强化"
    ...
    elif any(word in text for word in ["外貌加分", "好看了"]):
        visual = "上身效果强化"
        framework = "结果型卖点"
    ...
    elif any(word in text for word in ["27年", "4500", "6300万", "国民", "门店", "女性选择", "品质", "国货品牌", "线下专柜"]):
        visual = "品牌实力背书"
        framework = "品牌信任建立"
    ...
    elif any(word in text for word in ["高定无痕", "无痕面料", "无痕"]):
        visual = "高定无痕面料"
        framework = "无痕外穿卖点"
        viewer = "关注薄衣服外穿是否会露边、勒痕或尴尬留印。"
    elif any(word in text for word in ["副乳", "侧收", "支撑", "腋下", "双c位", "软支撑", "稳稳承托"]):
        visual = "侧收软支撑证明"
        framework = "支撑结构拆解"
...
def is_protected_fast_proof(shot: dict) -> bool:
    notes = str(shot.get("notes", ""))
    return "manual_added_fast_single_frame_proof" in notes or "protected_display_text_or_color_change" in notes


def should_merge_same_subtitle(previous: dict, subtitle: str, evidence_path: Path, shot: dict) -> bool:
    if previous.get("protected_fast_proof") or is_protected_fast_proof(shot):
        return False
    if not subtitle or subtitle != previous.get("raw_copy"):
        return False
    distance = visual_distance(Path(previous["evidence_frame"]), evidence_path)
    return distance < 0.36


def renumber_rows(rows: list[dict]) -> None:
    for index, row in enumerate(rows, start=1):
        shot_id = f"ModelShot_{index:03d}"
        row["shot_id"] = shot_id
        row["thumb_path"] = str(make_thumb(Path(row["evidence_frame"]), shot_id))
...
def build_rows(video_link: str = "") -> list[dict]:
    ...
    for index, shot in enumerate(report["shots"], start=1):
        ...
        if rows and should_merge_same_subtitle(rows[-1], subtitle, evidence_path, shot):
        ...
        rows.append(
            {
                ...
                "protected_fast_proof": is_protected_fast_proof(shot),
                **plan,
            }
        )
        if subtitle:
