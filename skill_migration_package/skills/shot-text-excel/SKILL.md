...
Default speed workflow: do not use `--full-ocr-sampling` on the first Excel generation unless the user explicitly asks for maximum recall. Generate with the specified `--subtitle-region`, `--disable-same-subtitle-merge` when alignment is required, and a bounded targeted budget. For normal bottom-subtitle videos use `--targeted-ocr-frame-budget 64`. For speed-sensitive bottom-subtitle videos that also contain fixed bottom disclaimers/watermarks, start with `--targeted-ocr-frame-budget 48`; this keeps near-end/start fallback but avoids spending time on many blank or disclaimer-only shots. If important subtitles are still missing, raise the budget to `64`, then `96`; use `--full-ocr-sampling` only as a last diagnostic pass.
