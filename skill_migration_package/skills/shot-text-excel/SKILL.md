...
2. Extract OCR text from shot evidence frames.

```powershell
python C:\Users\Administrator\.codex\skills\shot-text-excel\scripts\extract_shot_text_ocr.py --output-dir .\output --videos 1-11 --mode reference --target-width 900 --csv-file .\output\video_shot_text_ocr.csv --json-file .\output\video_shot_text_ocr.json
```
...
3. Prepare thumbnails for embedded workbook images.

```powershell
python C:\Users\Administrator\.codex\skills\shot-text-excel\scripts\prepare_ocr_thumbnails.py --root .
```
...
4. Build one Excel workbook per video.

```powershell
node C:\Users\Administrator\.codex\skills\shot-text-excel\scripts\build_individual_shot_text_excels.mjs --root .
```
...
```powershell
node C:\Users\Administrator\.codex\skills\shot-text-excel\scripts\build_individual_shot_text_excels.mjs --root . --video-link "<user provided video link line>"
```
...
For the current `E:\USE\codexhome\fenge` project, prefer the unified Python builder for new one-off videos instead of creating another per-video script:

```powershell
python E:\USE\codexhome\fenge\build_shot_text_excel_unified.py --video-file "<video.mp4>" --output-dir E:\USE\codexhome\fenge\output\test --excel-file "<optional output.xlsx>" --video-link "<optional link text>"
```

The unified builder automatically uses `model_calibrated` first, then `reference_optimized`, then `model_optimized`; extracts only bottom white subtitles; merges adjacent same-subtitle similar frames; preserves protected rapid proof frames; embeds images horizontally; and keeps the workbook row labels fixed.
...
Default speed workflow: do not use `--full-ocr-sampling` on the first Excel generation unless the user explicitly asks for maximum recall. Generate with the specified `--subtitle-region`, `--disable-same-subtitle-merge` when alignment is required, and the default `--targeted-ocr-frame-budget 64`. This budget samples empty shots in a round-robin order, prioritizing near-end and start frames so short subtitle windows are recovered without returning to the old 10-18 minute runtime. If important subtitles are still missing, raise the budget first, for example `--targeted-ocr-frame-budget 96`; use `--full-ocr-sampling` only as a last diagnostic pass.
...
```powershell
python E:\USE\codexhome\fenge\build_shot_text_excel_unified.py --video-file "E:\USE\codexhome\fenge\videos\test\5.8+文胸+都市丽人经典官方+11名.mp4" --output-dir "E:\USE\codexhome\fenge\output\test" --report-mode reference --disable-same-subtitle-merge
```
