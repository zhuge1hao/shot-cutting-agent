...
def suppress_continuous_portrait_duplicates(
    ...
    def shot_has_face(shot: dict[str, Any]) -> bool:
        ...
        if frame_no not in face_cache:
            face_cache[frame_no] = has_visible_face(read_video_frame(video_path, frame_no))
    ...
    def shot_feature(shot: dict[str, Any]) -> tuple[np.ndarray, np.ndarray] | None:
        ...
        if frame_no not in feature_cache:
            frame = read_video_frame(video_path, frame_no)
...
def process_video(video_path: Path, output_root: Path, reference_img_dir: Path | None = None) -> dict[str, Any]:
...
def main() -> None:
    ...
    summary = []
    for video in videos:
        report = process_video(video, output_dir, reference_img_dir)
        summary.append(
            {
